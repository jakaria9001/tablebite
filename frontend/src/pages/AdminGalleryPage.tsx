      ...
      <main className="px-5 py-10 sm:px-8 lg:px-10">
        <div className="mx-auto flex max-w-7xl flex-col gap-6">
          ...
          <div>
            <section className="rounded-[2rem] border border-black/5 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div><h2 className="text-xl font-black">Gallery</h2><p className="mt-1 text-sm text-slate-600">Drag to reorder. The homepage uses a random selection of these images.</p></div>
                  <button type="button" onClick={() => { setForm(emptyForm()); setEditingId(null); setFormOpen(true); }} className="min-h-11 rounded-full bg-[var(--maroon-800)] px-5 py-2 text-sm font-bold text-white">Add image</button>
                </div>
              </div>
            ...
            {formOpen ? <div className="fixed inset-0 z-50 overflow-y-auto bg-[var(--maroon-950)]/70 px-4 py-6 backdrop-blur-sm sm:py-10" role="dialog" aria-modal="true" aria-labelledby="gallery-form-title">
            <section className="mx-auto max-w-xl rounded-[2rem] border border-[var(--line)] bg-[var(--cream)] p-6 shadow-[var(--shadow-lifted)] sm:p-8">
              <div className="flex items-center justify-between">
                <h2 id="gallery-form-title" className="text-xl font-black text-[var(--maroon-900)]">{editingId ? "Edit Image" : "Add Image"}</h2>
                <button type="button" onClick={resetForm} aria-label="Close gallery form" className="grid h-11 w-11 place-items-center rounded-full border border-[var(--line)] bg-white text-xl">×</button>
              </div>
